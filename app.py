...
# ---------- THEME TOGGLE (icono sol/luna en la esquina superior derecha) ----------
from pathlib import Path
import streamlit.components.v1 as components

STATIC_DIR = Path(__file__).parent / "static"
DARK_CSS = STATIC_DIR / "style_dark.css"
LIGHT_CSS = STATIC_DIR / "style_light.css"

# Inicializar estado si no existe
if "theme" not in st.session_state:
    st.session_state.theme = "dark"   # valor por defecto: "dark"

def inject_css_from_file(path: Path):
    """Inyecta el contenido CSS del archivo en la página (robusto)."""
    try:
        css_text = path.read_text(encoding="utf-8")
        components.html(f"<style>{css_text}</style>", height=0)
        return True
...
def apply_theme(theme_name: str):
    """Aplica el tema cargando el CSS correspondiente."""
    if theme_name == "dark" and DARK_CSS.exists():
        inject_css_from_file(DARK_CSS)
    elif theme_name == "light" and LIGHT_CSS.exists():
        inject_css_from_file(LIGHT_CSS)
    else:
        # fallback: intenta cargar dark si no encuentra el solicitado
        if DARK_CSS.exists():
            inject_css_from_file(DARK_CSS)

# --- UI: mostrar toggle en la esquina superior derecha ---
# Insertar en la misma fila de header: hacemos 2 columnas, la segunda pequeña para el toggle
col_left, col_right = st.columns([9, 1])  # ajusta proporciones a tu layout
with col_right:
    # Mostrar el ícono acorde al tema actual
    ...
    # Botón que cambia tema (usa key distinto para evitar conflictos)
    if st.button(f"{icon}  ", key="toggle_theme", help=f"Cambiar a {('claro' if st.session_state.theme=='dark' else 'oscuro')}"):
        # alternar
        st.session_state.theme = "light" if st.session_state.theme == "dark" else "dark"
        # fuerza rerun (Streamlit reruns después del evento del botón automáticamente)

# Aplicar el tema actual (siempre al render)
